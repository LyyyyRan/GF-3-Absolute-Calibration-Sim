...
Profile_Position = [872 - 1, 1914 - 1]  # matlab from 1 on, python from 0 on
...
ROI_f_Padded = np.concatenate((ROI_f[:, 0:20], X_Insert, ROI_f[:, 20:33]), axis=1)
...
# 方位向信号参数
alpha_os_a = 1.23  # 1.7  # 方位过采样率(高过采样率避免鬼影目标)
Naz = 1600  # 距离线数
...
ROI_BBox_show = cv.rectangle(ROI_BBox_show, (190, 0), (225, Na - 1), color=(255, 0, 0), thickness=1)  # Azimuth longer
ROI_BBox_show = cv.rectangle(ROI_BBox_show, (0, 150), (Nr - 1, 222), color=(255, 0, 0), thickness=1)  # Range longer
...
# the Setting RCS:
RCS = eval(mat_path.split('RCS_')[-1].split('_')[2])
...
# print('Origin ROI Shape:', ROI.shape)
# print('Upsampled ROI Shape:', ROI_Modulus.shape)
# print('N of Upsample:', N_factor)
